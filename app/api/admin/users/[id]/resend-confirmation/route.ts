...
const ACTION = "resend_confirmation";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Servidor busca o e-mail pelo user_id — nunca aceita e-mail vindo do
  // cliente, mesmo que pareça o mesmo.
  ...
  if (email_confirmed_at) {
    return Response.json({ error: "Este e-mail já está confirmado." }, { status: 409 });
  }
  ...
  const { error: resendError } = await service.auth.resend({ type: "signup", email });
  if (resendError) {
    await logAdminAction({ actorUserId: admin.userId, targetUserId, action: ACTION, result: "error", safeMetadata: { message: resendError.message } });
    return Response.json({ error: "Não foi possível reenviar a confirmação." }, { status: 500 });
  }
